...
class Plot:
    ...
    def plot_surface_of_type_price(self, df):
        ...
        df = df[(df['home_type'] == 'Appartement') | (
            df['home_type'] == 'Villa') | (df['home_type'] == 'Maison')]
        ...
        plt.xscale('log')
        plt.yscale('log')
        scatter.set_axis_labels('Prices of properties',
                                'Surface of properties')
    ...
    def distribution_of_surface(self, df):
        ...
        df = df[(df['home_type'] == 'Appartement') | (
            df['home_type'] == 'Villa') | (df['home_type'] == 'Maison')]
        df = df[df['surface_of_land_area'] > 0]
        first = (df['surface_of_land_area'] > (
            df['surface_of_land_area'].mean())+500).sum()
        second = (df['surface_of_land_area'] < (
            df['surface_of_land_area'].mean())-500).sum()
    ...
    def compare_state_of_building_withprice(self, df):
        ...
        df = df[(df['state_of_building'] == 'Bien neuf') | (df['state_of_building'] == 'Besoin de travaux') | (
            df['state_of_building'] == 'Rénové') | (df['state_of_building'] == 'Bon')]
        df = df[df['price'] > 0]
        ax = sns.stripplot(x='state_of_building', y='price', data=df,
                           jitter=0.4, size=10,  hue='state_of_building', linewidth=1)
    ...
    def price_in_flandre(self, df):
        ...
        df = df[(df['locality'] == 'Hal-Vilvorde') | (df['locality'] == 'Audenarde') | (df['locality'] == 'Hasselt') | (df['locality'] == 'Gand') |
                (df['locality'] == 'Alost') | (df['locality'] == 'Ostende') | (df['locality'] == 'Huy') | (df['locality'] == 'Bruges') | (df['locality'] == 'Anvers')]
        df = df[df['price'] > 0]
        ax = sns.stripplot(x='locality', y='price', data=df,
